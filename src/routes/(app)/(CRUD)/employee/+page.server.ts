...
export const load: PageServerLoad = async ({ url, cookies }) => {
  ...
  const count = await axios.get(`${PUBLIC_API_ENDPOINT}/employee`, {
  ...
  const data = await fetch(`${PUBLIC_API_ENDPOINT}/employee?page=${page}`, {
    headers: customHeaders,
  });
  const res = data.json();
  return {
...
export const actions = {
  create: async ({ request, cookies }) => {
    ...
    try {
      axios
        .post(`${PUBLIC_API_ENDPOINT}/employee`, data, {
          headers: customHeaders,
        })
  ...
  delete: async ({ request, cookies }) => {
    ...
    try {
      axios
        .delete(`${PUBLIC_API_ENDPOINT}/employee/` + data.get("id"), {
          headers: customHeaders,
        })
        .then((response) => {
          return {
  ...
  edit: async ({ request, cookies }) => {
    ...
    try {
      axios
        .put(`${PUBLIC_API_ENDPOINT}/employee/${data.get("id")}`, data, {
          headers: customHeaders,
        })
        .then(function (response) {
          console.log(response);
